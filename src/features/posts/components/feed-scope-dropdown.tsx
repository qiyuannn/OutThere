...
const options: readonly FeedScope[] = ['explore', 'following'];

export function FeedScopeDropdown({ value, open, onOpenChange, onChange }: FeedScopeDropdownProps) {
  return (
    <View style={styles.wrapper}>
      <Pressable
        accessibilityLabel={`Show ${labelForFeedScope(value)} feed`}
      ...
      >
        ...
        <ThemedText style={styles.triggerLabel}>{labelForFeedScope(value)}</ThemedText>
      </Pressable>
        ...
        <View accessibilityRole="menu" style={styles.menu}>
          {options.map((option) => (
            <Pressable
              ...
              style={({ pressed }) => [
                styles.menuItem,
                value === option && styles.menuItemSelected,
                pressed && styles.pressed,
              ]}
            >
              <ThemedText style={styles.menuLabel}>{labelForFeedScope(option)}</ThemedText>
            </Pressable>
          ))}
        </View>
      ) : null}
